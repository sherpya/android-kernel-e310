...
// for Tcxo shutdown
// don't enter txco shutdown if DUT receive character from uart3 rx
...
void arch_idle(void)
{
	...
	if ((timer_expiration < msm_pm_idle_sleep_min_time) ||
		...
		(has_wake_lock(WAKE_LOCK_SUSPEND) && !wake_lock_active(&main_wake_lock)) || // for Tcxo shutdown
#endif
	...
	if( msm_pm_debug_mask & MSM_PM_DEBUG_IDLE ) {
		printk("timer_expiration=%lld  msm_pm_idle_sleep_min_time=%d  IDLE_WAKE_LOCK=%ld  msm_irq_idle_sleep_allowed=%d\r\n",
			timer_expiration, msm_pm_idle_sleep_min_time, has_wake_lock(WAKE_LOCK_IDLE), msm_irq_idle_sleep_allowed());
	}
	...
	for (i = 0; i < ARRAY_SIZE(allow); i++)
		MSM_PM_DPRINTK(MSM_PM_DEBUG_IDLE, KERN_INFO,
			...
			msm_pm_sleep_mode_labels[i], (int)allow[i]);
          	...
          	ktime_get_ts(&now);  // for Tcxo shutdown , enter power collapse after 60 seconds
    
	if (now.tv_sec >= 60 && (allow[MSM_PM_SLEEP_MODE_POWER_COLLAPSE] ||
		allow[MSM_PM_SLEEP_MODE_POWER_COLLAPSE_NO_XO_SHUTDOWN])) {
...
#else 
    sleep_delay = (uint32_t) msm_pm_convert_and_cap_time(
			tcxo_sleep_time/*500000000*/, MSM_PM_SLEEP_TICK_LIMIT);  // for Tcxo shutdown
#endif
       ...
       // for Tcxo shutdown
#ifdef CONFIG_MSM_IDLE_STATS
		are_all_clocks_disabled |= cci_check_all_enabled_clock();
		are_all_clocks_disabled = !are_all_clocks_disabled;
		if( !are_all_clocks_disabled ||
		     (touch_workaround_sleep_time <= 0 && (t1 - Touch_last_time) < 2000000000)  ||   // 2000 ms
...
if (ret)
			...
			if( (msm_irq_pending() & (1U << 30)) ) {
				...
				Touch_last_time = ktime_to_ns(ktime_get());
			}
		...
		if( (int64_t) t3 > 0 ) {
			//printk("t1=%lld  t3=%lld  comp=%lld\r\n", t1, t3, enter_tcxo_time_limit);
		...
		if( cci_check_enabled_clock(14) )  // MDP clock
			tcxo_sleep_time = timer_expiration;

//		if( cci_check_enabled_clock(19) )  // P_SDC1_CLK
//			tcxo_sleep_time = timer_expiration;

		if( touch_workaround_sleep_time > 0 ) {
			tcxo_sleep_time = RESET_TCXO_SLEEP_TIME;
			touch_workaround_sleep_time -= RESET_TCXO_SLEEP_TIME;
			if( touch_workaround_sleep_time <= 0 )
				power_collapse_ret = 0;
		}
		...
		if( (msm_irq_pending() & (1U << 30)) ) {
			Touch_last_time = ktime_to_ns(ktime_get());
			if( touch_workaround_sleep_time <= 0 && power_collapse_ret )
				touch_workaround_sleep_time = RESET_TCXO_TOTAL_SLEEP_TIME;
		}

		if( msm_pm_debug_mask & MSM_PM_DEBUG_IDLE )
			printk("#######  tcxo_sleep_time=%lld ##### ret=%d #######\r\n", tcxo_sleep_time, ret);
...
#ifdef original
#else
        suspend_clk_check = cci_check_suspend_enabled_clock();
#endif				
...
enum {
    ...
    RPC_REBOOT_TYPE_SD_CLEAN_DLOAD,
    RPC_REBOOT_TYPE_FERRARI_CHECK_FAIL,
    RPC_REBOOT_TYPE_BOOT_MENU,
};
...
static void msm_pm_restart(char str, const char *cmd)
{
	...
	else if(restart_reason == 0x77665588) { 
	    err = cci_rpc_reboot(RPC_REBOOT_TYPE_ACER_FOTA_OS_UG);
	}
	else if(restart_reason == 0x77665555) { 
	    err = cci_rpc_reboot(RPC_REBOOT_TYPE_SD_CLEAN_DLOAD);
	} 
    else if(restart_reason == 0x77665504) {   
	    err = cci_rpc_reboot(RPC_REBOOT_TYPE_FERRARI_CHECK_FAIL);  
    } 
    else if(restart_reason == 0x77665577) {
	    err = cci_rpc_reboot(RPC_REBOOT_TYPE_BOOT_MENU);
    }
    else {
	msm_rpcrouter_close();
	...
	(struct notifier_block *this, unsigned long code, void *_cmd)
{
	if ((code == SYS_RESTART) && _cmd) {
		...
		} else if (!strcmp(cmd, "sd_download")){
			 restart_reason = 0x77665544;
		} else if (!strncmp(cmd, "oem-", 4)) {
			unsigned code = simple_strtoul(cmd + 4, 0, 16) & 0xff;
			restart_reason = 0x6f656d00 | code;
		} else if (!strcmp(cmd, "fota_os_ug")){ 
			restart_reason = 0x77665588;
		} else if (!strcmp(cmd, "sd_clean_download")){ 
			restart_reason = 0x77665555;
		} else if (!strcmp(cmd, "FERRARI_CHECK_FAIL")){   
			restart_reason = 0x77665504;  
		} else if (!strcmp(cmd, "boot_menu")){  
			restart_reason = 0x77665577;  
		}  
		else {
...
static int __init msm_pm_init(void)
{
	...
	*pmd = __pmd((virt_to_phys(msm_pm_collapse_exit) & PGDIR_MASK) |
	...
	comp_time.tv_sec = 10;   // 10 seconds
	comp_time.tv_nsec = 0;
	enter_tcxo_time_limit = ktime_to_ns(timespec_to_ktime(comp_time));

	comp_time.tv_sec = 10;   // 10 seconds
	comp_time.tv_nsec = 0;
	uart_receive_rx_time_limit = ktime_to_ns(timespec_to_ktime(comp_time));  // Uart3 rx delay time
