...
void msm_irq_priority_set(unsigned long int_number,unsigned long level)
{
	printk("@@msm_irq_priority_set,irq_num:%ld ,level:%ld\n",int_number,level);
	if(level>7)
		return;
	return	writel(level,VIC_VECTPRIORITY(int_number));
}

unsigned long msm_irq_priority_get(unsigned long int_number)
{
	return	readl(VIC_VECTPRIORITY(int_number));
}
