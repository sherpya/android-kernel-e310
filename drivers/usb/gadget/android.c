...
//use IMEI as USB serial number
static char usb_imei[16] = {0};
static char dft_SerialNum[]="1234567890ABCDEF";
        ...
        unsigned long count, void *data)
{
    ...
    if ( buf[0] == '1' ) 
    {
        strcpy(serial_number, dft_SerialNum);
        printk(KERN_INFO "Change USB S/N to %s\n", serial_number);
        strings_dev[STRING_SERIAL_IDX].s = serial_number;
    }
    else 
    {
        strcpy(serial_number, usb_imei);
...
//use IMEI as USB serial number, end
...
static int __init android_probe(struct platform_device *pdev)
{
	...
	printk(KERN_ERR "[USB]android_probe");
    ...
    //use IMEI as USB serial number
    {
        /*
        [Description]
        Copy IMEI to share memory depends on detection of NV item engineer mode.
        Check condition:
        1.	NV Engineer mode == 0: read NV item IMEI if success set cci_imei_num_ready = 0x01 and ARM11 ready to read IMEI from share memory.
        2.	NV Engineer mode == 0: read NV item IMEI if No active or fail set cci_imei_num_ready = 0x02 and ARM11 skip to read IMEI from share memory.
        3.	NV Engineer mode == 1: set cci_imei_num_ready = 0x03 and ARM11 skip to read IMEI from share memory.
        4.	NV Engineer modes == No active or read fails set cci_imei_num_ready = 0x03 and ARM11 skip to read IMEI from share memory.
        5.	cci_imei_num_ready default value is 0xFF ARM11 need to wait for cci_imei_num_ready change.
        */
        ...
        {
            if( smem_cci_smem_value->cci_imei_num_ready == 0x01) 
            {
                printk("cci_imei_num_ready = 0x01\n");

                memcpy(usb_imei, smem_cci_smem_value->cci_imei_num, 15);
                usb_imei[15]='\0';
            ...
            else if( smem_cci_smem_value->cci_imei_num_ready == 0x02) 
            {
                printk("cci_imei_num_ready = 0x02\n");
                break;
            } 
            else if( smem_cci_smem_value->cci_imei_num_ready == 0x03) 
            {
                printk("cci_imei_num_ready = 0x03\n");
                break;
            } 
            else 
            {
                msleep(100);
                printk("cci_imei_num_ready = 0xFF\n");
            }
        }
	...
	if (pdata) {
		pdata->vendor_id = C4_VID;
		strcpy(pdata->product_name, C4_PRODUCT_NAME);
		strcpy(pdata->manufacturer_name, C4_MANUFACTURER_NAME);
		...
		printk(KERN_ERR "[USB]device_desc.idVendor=%d, device_desc.idProduct=%d", \
