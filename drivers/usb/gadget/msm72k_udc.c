...
{
	unsigned long flags;
	printk(KERN_ERR "[USB]msm_hsusb_set_state:%d\n", state);
...
static enum usb_device_state msm_hsusb_get_state(void)
{
	...
	spin_unlock_irqrestore(&the_usb_info->lock, flags);
	printk(KERN_ERR "[USB]msm_hsusb_get_state:%d\n", state);
...
static int usb_get_max_power(struct usb_info *ui)
{
	...
	int configured;
	unsigned bmaxpow;
	printk(KERN_ERR "[USB]usb_get_max_power\n");
...
static int usb_phy_stuck_check(struct usb_info *ui)
{
	 ...
	 */
	 printk(KERN_ERR "[USB]usb_phy_stuck_check\n");
...
static void usb_phy_stuck_recover(struct work_struct *w)
{
	...
	struct msm_otg *otg = to_msm_otg(ui->xceiv);
	unsigned long flags;
	printk(KERN_ERR "[USB]usb_phy_stuck_recover\n");
...
static void usb_chg_stop(struct work_struct *w)
{
	...
	struct msm_otg *otg = to_msm_otg(ui->xceiv);
	enum chg_type temp;
	printk(KERN_ERR "[USB]usb_chg_stop\n");
...
static void usb_chg_detect(struct work_struct *w)
{
	...
	unsigned long flags;
	int maxpower;
	printk(KERN_ERR "[USB]usb_chg_detect\n");
...
static void usb_do_work_check_vbus(struct usb_info *ui)
{
	...
	printk(KERN_ERR "[USB]usb_do_work_check_vbus\n");

	spin_lock_irqsave(&ui->lock, iflags);
	printk(KERN_ERR "[USB]is_usb_online=%d\n",is_usb_online(ui) );
	if (is_usb_online(ui))
	...
	for (;;) {
		...
		case USB_STATE_ONLINE:
			...
			if (flags & USB_FLAG_VBUS_OFFLINE) {
				printk(KERN_ERR "[USB]USB_FLAG_VBUS_OFFLINE\n");
				...
				spin_unlock_irqrestore(&ui->lock, iflags);
				printk(KERN_ERR "[USB]cci_android_charger_usb_change(0, 0);\n");
...
void msm_hsusb_set_vbus_state(int online)
{
	unsigned long flags;
	struct usb_info *ui = the_usb_info;
	printk(KERN_ERR "[USB]msm_hsusb_set_vbus_state: online=%d\n", online);
