...
static int msm_otg_resume(struct msm_otg *dev)
{
	...
	if (device_may_wakeup(dev->otg.dev)) {
		pr_info("[USB]device_may_wakeup\n");
		disable_irq_wake(dev->irq);
		if (dev->vbus_on_irq)
		{
			pr_info("[USB]disable_irq_wake\n");
...
{
	pr_info("[USB]msm_otg_get_resume\n");
...
static int msm_otg_set_suspend(struct otg_transceiver *xceiv, int suspend)
{
	...
	pr_info("[USB]suspend request in state: %s %d\n",
			state_string(state), suspend);
	...
	if (suspend) {
		...
		pr_info("[USB]msm_otg_set_suspend-!suspend\n");
...
static int msm_otg_runtime_suspend(struct device *dev)
{
	...
	pr_info("[USB]pm_runtime: suspending...\n");
	msm_otg_suspend(otg);
	return  0;
}
...
static int msm_otg_runtime_resume(struct device *dev)
{
	...
	pr_info("[USB]pm_runtime: resuming...\n");
	msm_otg_resume(otg);
	return  0;
}
