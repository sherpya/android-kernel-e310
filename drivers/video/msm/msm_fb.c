...
cci_smem_value_t *smem_cci_smem_value=NULL;
//extern cci_smem_value_t *smem_cci_smem_value;
...
static ssize_t msm_fb_write(struct fb_info *info, const char __user *buf, size_t count, loff_t *ppos);
...
static void msm_fb_imageblit(struct fb_info *info, const struct fb_image *image)
{
	...
	if (!mfd->hw_refresh && (info->var.yoffset == 0) &&
		...
		//printk("[Aaron][%s] image->dx=%d, image->dy=%d\r\n", __func__, image->dx, image->dy);
...
#ifdef CONFIG_FB_MSM_LOGO
	//if (info->fbops->fb_open(info, 0))
	//if (fb_open(fbi, 0))
	if (fbi->fbops->fb_open(fbi, 0))
	{
		printk("[Aaron][%s] fb_open(fbi, 0) fail\r\n", __func__);
	}
	else
	{
		printk("[Aaron][%s] fb_open(fbi, 0) success\r\n", __func__);
	}

	//if (!load_565rle_image(INIT_IMAGE_FILE)) ;	/* Flip buffer */

	if (!smem_cci_smem_value)
	{
		smem_cci_smem_value = smem_alloc( SMEM_CCI_SMEM_VALUE, sizeof( cci_smem_value_t ));
	}
	
	//printk("[John][%s] operator_ID=%d\r\n", __func__, smem_cci_smem_value->operator_ID);
	// operator_ID
	// 1 FET
	// 2 GEN1
	// 3 SFR
...
msm_fb_read(struct fb_info *info, char __user *buf, size_t count, loff_t *ppos)
{
	...
	if ((smem_cci_smem_value->cci_project_id == PROJECT_ID_CAP8) ||
		(smem_cci_smem_value->cci_project_id == PROJECT_ID_K4))
	{
		src = (u32 __iomem *) (info->screen_base + p + ((p/480)*32));
	}
	else
	{
	...
	while (count) {
		...
		for (i = c >> 2; i--; )
		{
			if ((smem_cci_smem_value->cci_project_id == PROJECT_ID_CAP8) ||
				(smem_cci_smem_value->cci_project_id == PROJECT_ID_K4))
			{
				if ((((u32)src + (u32)32) % (u32)512)  < (u32)32)
				{
					src = (u32 __iomem *)((char *)src + 32);
				}
			}
...
static ssize_t
msm_fb_write(struct fb_info *info, const char __user *buf, size_t count, loff_t *ppos)
{
	unsigned long p = *ppos;
	u32 *buffer, *src;
	u32 __iomem *dst;
