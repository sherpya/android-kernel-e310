...
dhdsdio_isr(void *arg)
{
	...
	if (bus->dhd->busstate == DHD_BUS_DOWN) {
		...
		if(busdown_isr_count < 10) {
			...
			bus->intdis = TRUE;
			DHD_ERROR(("%s : bus is down. bcmsdh_intr_disable\n", __FUNCTION__));
			busdown_isr_count = 0;
			return;
		}
	}
...
dhdsdio_disconnect(void *ptr)
{
	...
	DHD_TRACE(("%s: Enter\n", __FUNCTION__));
	DHD_ERROR(("%s: busdown_isr_count = 0\n", __FUNCTION__));
	busdown_isr_count = 0;
	if (bus) {
		ASSERT(bus->dhd);
...
int DumpArray(char* buf, int len)
{
    int iStrIndex = 0;
    int i;
    char zSTR[81];
