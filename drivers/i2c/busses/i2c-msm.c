...
//extern void msm_i2c_error_log(unsigned char *str);
unsigned long I2C_fail_cnt=0;
...
wait_for_int:
		...
		if (!timeout) {
			...
			sprintf(str_buff,"I2c xfr time out,addr:0x%x,cnt:%ld\n",addr,I2C_fail_cnt);
			//msm_i2c_error_log(str_buff);
			I2C_fail_cnt++;
			writel(I2C_WRITE_DATA_LAST_BYTE,
				dev->base + I2C_WRITE_DATA);
			msleep(100);
		...
		if (dev->err) {
			...
			sprintf(str_buff,"I2c xfr error,addr:0x%x,err:%d,cnt:%ld\n",addr,dev->err,I2C_fail_cnt);
			//msm_i2c_error_log(str_buff);
			I2C_fail_cnt++;
